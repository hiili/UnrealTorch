...
utility.LUA_LOG = (
  function()
    local ELogVerbosity_inv = tomap( map( function(k,v) return v, k end, getmetatable(ELogVerbosity).__index ) )
...
function utility.redirect_output()
  ...
  UE_LOG( ELogVerbosity.Log, 'Redirecting Lua print() and io.write() to \'' .. logfile .. '\'' )
  print = utility.print
  io.output( logfile )
...
function utility.log_function_start()
  ...
  LOG( ELogVerbosity.VeryVerbose, message )
...
--                      Increase to start from a more distant upfunction.
